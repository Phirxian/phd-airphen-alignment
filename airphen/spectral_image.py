    ...
    def spectral_registration(self, config):
        ...
        if method is None:
            ...
            if reference == None:
                ...
                iterator = list(pair_band_iterator(len(self.registred)))
            else:
                iterator = [(i, reference) for i in range(len(self.registred))]
                sink = reference

            # remove self assignement
            iterator = [i for i in iterator if i[0] != i[1]]

            self.registred, bbox, nb_kp, centers = multiple_refine_allignement(
                self, self.registred, config, iterator, sink
            )
