...
\begin{document}
	...
	\abstract{
		...
		\\
		In this study we propose a two-step method applied to the six-bands Airphen multi-sensors camera with
		...
		and the best spectral reference was identified to be the 570 nm for this one.
...
%	(NDVI \footnote{Normalized Difference Vegetation Index}, ExcessGreen, \dots)
	...
	The multi-spectral imagery is provided by the six-band multi-spectral camera Airphen developed by HiPhen society.
	...
	The size of the gantry is $3m \times 5m \times 4m$.
	...
	we have bounded the acquisition height from $1.6$ m to $5$ m with $20$ cm steeps, which represent 18 acquisitions.
	\\
	...
	Figure \ref{fig:merged-correction} shows a closeup inside at 1.6 m (\ref{fig:merged-correction-uncorrected}) unregistred image,
	...
	the figure \ref{fig:affine-error} show an example using 570 nm as reference before the perspective correction.
	...
	\\
	\par All methods show that the best spectral band is 710 nm (in red), with the exception for SURF and GFTT which is 570 nm.
	...
	the same behavior can be observed for the other bands and 570 nm.
	...
	\begin{figure}[h]
		\centering
		\includegraphics[height=0.5\linewidth, angle=90]{../figures/prespective-allignement-rmse.jpg}
