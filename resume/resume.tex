...
\begin{document}
	...
	\begin{abstract}
		...
		and is currently based on the use of multi-sensor cameras. However, their development usually concerns aerial application and their parameters are optimized for high altitudes acquisition by drone ($\approx 50$ meters)
...
%	(NDVI \footnote{Normalized Difference Vegetation Index}, ExcessGreen, \dots)
	...
	\begin{figure}[H]
		...
		\caption{Each corrected step, with (1) roughly corrected from affine correction and (2) enhancement via key-points and perspective}
	...
	We use opencv 4 \textit{findChessboardCorners} for each spectral image (normalized) at different heights (from $1.6\si{\meter}$ to $5\si{\meter}$).
	...
	(\SI{1.6}{\meter} where the spatial resolution of the chessboard is higher), because it does not depend on the height.
	...
	\\
	\par All methods show that the best reference spectrum is \SI{710}{\nano\meter} (in red), with the exception of SURF and GFTT which is \SI{570}{\nano\meter}.
	The figure \ref{fig:features-GFTT-performances} shows the minimum number of matches between each reference spectrum and all others, for each relevant methods.
	...
	\\
	\par \colorbox{red}{parler erreur avant-arpès et constructeur}
	%\\
	...
	with a significant number of matches $150-450$ and a reasonable calculation time \SI{1.17}{\second} to \SI{3.55}{\second}.
	\\
